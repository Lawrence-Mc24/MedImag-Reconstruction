...
def compton_angle(E_initial, E_final):
    ...
    energy (Joules). Input takes eV?'''
    # May not need e factor depending on detector energy output
    E_initial = E_initial*e
    E_final = E_final*e
    initial = h*c/E_initial
    final = h*c/E_final
    print(1- m_e*(c**2)*(1/E_final - 1/E_initial))
    angle = np.arccos(1- m_e*(c**2)*(1/E_final - 1/E_initial))
    return angle
...
def x_prime_y_prime_single(z_prime, theta, phi, alpha, r1):
    
    a = np.tan(alpha)    
    z_prime = z_prime - r1[2]
    #i is our psi variable
...
def simulate(E, r_source, d12):
    ...
    ----------
    E : TYPE - value
        DESCRIPTION - energy of source 
    r_source : TYPE - array-like
        DESCRIPTION - 3 (x, y, z) co-ordinates of point-source location 
    d12 : TYPE - value
        DESCRIPTION - distance between detectors in z direction

    Returns
    -------
    r1, r2
        DESCRIPTION - two arrays of detector hits

    '''
    r1 = np.array([random.uniform(-5, 5), random.uniform(-5, 5), 0])
    E = E*e
    E_f = random.uniform(E/(1 + 2*E/(m_e * c**2)), E)
    print(E/(1 + 2*E/(m_e * c**2)))
    print(f'final energy: {E_f}')
    angle = compton_angle(E/e, E_f/e)
    v1 = r1 - r_source
    theta = theta_angle(r1[0], r_source[0], r1[1], r_source[1], r1[2], r_source[2])
    phi = phi_angle(N(v1))
    x, y = x_prime_y_prime_single(-d12, theta, phi, angle, r1)
    r2 = np.array([x, y, -d12])
    return [r1, r2, angle]


points = simulate(600000, np.array([3, 2, 1]), 1)
...
#print(f'xy3 = {xy3}')
#print(xy1)
xys = give_x_y_for_two_points(r1, r2, z_prime=1, alpha=points[2], steps=180, estimate=1)
print(f'xys: {np.size(xys[0])}')
#plot_it2(xys, np.array([r1, r3, r5]), individual_points=True)
plot_it(xys[0], xys[1], r1=r1, individual_points=False)